...
Analyze the conversation context and return the structured JSON object.`;
    ...
    const response = await this.groqClient!.chat.completions.create({
      ...
      temperature: 0.2,
      max_tokens: 1500, // Generous limit so reasoning tokens do not cut off output
    });

    const rawContent = response.choices[0]?.message?.content?.trim() ?? '';
    const parsed = this.parseGroqJson(rawContent);

    // Normalize budget to a clean number in rupees
  ...
  /**
   * Robust JSON parser for Groq model responses
   */
  ...
  /**
   * Helper to normalize budget values into a numeric rupee value
   */
  private normalizeBudget(val: any): number | null {
    if (typeof val === 'number') {
      if (val > 100000) return Math.round(val);
      if (val > 0 && val <= 100) return Math.round(val * 10000000); // e.g. 1.2 or 1.5 Cr
    }
  ...
  /**
   * Intelligent Real-Estate Rule-Based Engine (Fallback)
   * Scans ONLY customer messages for BHK/budget to prevent false extractions.
   */
  private fallbackAnalysis(
    ...
    // Extract text ONLY from customer messages + current message
    ...
    // Extract configuration from customer messages
    const bhkMatch = customerAllText.match(/\b([1-5])\s*(?:bhk|bedroom|rk)\b/i);
    const configuration = bhkMatch ? `${bhkMatch[1]} BHK` : null;

    // Extract budget from the MOST RECENT customer mention first, then earlier
    ...
    // Formulate natural conversational reply step-by-step
